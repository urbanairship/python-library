...
def web(alert=None, extra=None, icon=None, title=None, interactive=None,
        time_to_live=None, interaction_required=None):
    ...
    :keyword title: Optional string. Represents the title of the notification.
    :keyword interactive: A optional dictionary with two attributes -- "type" and
        "button_actions", as generated by :py:func:`interactive`
    :keyword time_to_live: An optional integer or time set in UTC as a string
    :keyword interaction_required: An optional boolean value that requires user
        interaction before notification is dismissed.
    ...
    if time_to_live is not None:
        if not (isinstance(time_to_live, (string_type, int))):
            raise ValueError('Web time_to_live must be an interger or string')
        payload['time_to_live'] = time_to_live
    if interaction_required is not None:
        if not (isinstance(interaction_required, (bool))):
            raise ValueError('interaction_required must be boolean type')
        payload['interaction_required'] = interaction_required
    return payload


def open_platform(alert=None, title=None, extra=None):
